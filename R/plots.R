...
#' # Load library and its dataset
#' library(smbinning)
#' smbmetricsdf <- smbinning.metrics(
#'   dataset = smbsimdf1, prediction = "cbs1",
...
smbinning.metrics.plot <- function(df, cutoff = NA, plot = "cmactual") {
  ...
  # Check if target variable is numeric
           ...
           (max(df$Prediction) < cutoff | min(df$Prediction) > cutoff)) {
    return("'cutoff' out of range.")
  }
  else if (plot != "cmactual" & plot != "cmactualrates" &
           plot != "cmmodel" & plot != "cmmodelrates") {
